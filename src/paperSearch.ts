import { Paper, SearchOptions, ConferencesConfig, DblpResponse, ConferenceInfo } from './types.js';
import conferencesData from './conferences.json' with { type: 'json' };

const conferences = conferencesData as ConferencesConfig;

// DBLP venue key mappings for accurate matching
const DBLP_VENUE_MAPPINGS: Record<string, string[]> = {
...
export function getAllConferences(): Record<string, ConferenceInfo> {
  return {
...
export function getConferencesByTier(tier: 'top' | 'second' | 'all'): Record<string, ConferenceInfo> {
  if (tier === 'top') {
...
export function getConferenceInfo(key: string): ConferenceInfo | undefined {
  const all = getAllConferences();
  return all[key.toLowerCase()];
}

// Get conference website URL for a specific year
export function getConferenceWebsiteUrl(key: string, year: number): string | undefined {
  const conf = getConferenceInfo(key);
  if (!conf) return undefined;
  return conf.urls[year.toString()];
}

function buildDblpQuery(options: SearchOptions): string {
  const parts: string[] = [];
...
function matchConference(venue: string): { key: string; conf: ConferenceInfo } | null {
...
export async function searchPapers(options: SearchOptions): Promise<Paper[]> {
...
export async function searchByConference(
...
export function listAvailableConferences(): {
  topTier: { key: string; name: string; shortName: string; years: string[] }[];
  secondTier: { key: string; name: string; shortName: string; years: string[] }[];
} {
